...
abstract class ViewsBulkOperationsBaseOperation {
  ...
  /**
   * Returns whether the operation is marked as 'vbo'. Used to differentiate it
   * from core actions that are processed in a different way.
   */
  public function isVboAction() {
    return !empty($this->operationInfo['vbo']);
  }
   ...
   *
   * The admin options form is embedded into the EBO field settings and used
   ...
   *   is constructed by the EBO views field handler, so it's not a real form
   ...
   * by the EBO views field handler, which also injects the options into the
